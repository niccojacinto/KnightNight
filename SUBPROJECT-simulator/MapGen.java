...
public class MapGen{
  ...
  private static void tryToGenerateHallAndRoom(Room currentRoom, Direction currentDirection, ArrayList<Room> openRooms,
    ...
    int roomShortSide = getRandomInt(ROOM_SHORT_SIDE_MIN_ADJUSTED, getRoomMaxAlong(currentDirection.getRotatedClockwise()));
    ...
    do
    {
      ...
      Hall overlapHall = new Hall(newHall.X()-1, newHall.Y() -1, newHall.width(), newHall.height(), newHall.direction());
      
      //4b. Generate Room 
      Room newRoom = generateRoomAt(newHall, currentDirection, roomShortSide, roomLongSide);
      Room overlapRoom = new Room(newRoom.X()-1, newRoom.Y() -1, newRoom.width(), newRoom.height());
  ...
  private static Room generateRoomAt(Hall h, Direction d, int shortSide, int longSide){
    switch (d){
      case LEFT:
        return new Room(h.X() - longSide + 1, h.Y(), longSide, shortSide);
      case RIGHT:
        return new Room(h.X() + h.width() - 1, h.Y(), longSide, shortSide);
      case UP:
        return new Room(h.X(), h.Y() - longSide + 1, shortSide, longSide);
      case DOWN:
        return new Room(h.X(), h.Y() + h.height() - 1, shortSide, longSide);
      default:
        throw new Error("Invalid direction supplied: " + d);
    }
  }
